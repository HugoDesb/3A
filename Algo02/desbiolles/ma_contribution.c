...
{ /* +/- 15 lignes. Il faudra retourner une valeur convenable. */
  ...
  for (i = 0; i < (taille_graphe(graphe) && symetrique); i++)
  {
    ...
    for (j = i; j < (taille_graphe(graphe) && symetrique); j++)
    {
      ...
      else if ((get_arc(graphe, i, j) || get_arc(graphe, j, i)) && symetrique)
      {
...
void parcours_profondeur_niveaux_rec(t_gra graphe, int sommet, int niveau) //DONE
{
  ...
  for (i = 0; i < taille_graphe(graphe); i++)
  {
    ...
    {
      tremper(graphe, i);
      //minimum entre le poids actuel et le niveau
      sommet_set_poids(graphe, i, min(niveau, sommet_get_poids(graphe, i)));
      //récursion
      parcours_profondeur_niveaux_rec(graphe, i, niveau + 1);

      //on le sèche pour pouvoir le réutiliser dans les prochains appels
      // de la récusrion dans la boucle
      secher(graphe, i);
    }
  }
}
...
{                                                              /* +/- 15 lignes. Il faudra retourner une valeur convenable. */
  ...
  for (v = 0; v < taille_graphe(graphe); v++)
  {
    ...
    if (!mouille(graphe, v))
    {
      ...
      {
        //itérativement, on vérifie que ses prédécesseurs sont tous secs
        // ou qu'il en ait au moins un
        toutPredecesseurMouille = toutPredecesseurMouille && (!get_arc(graphe, u, v) || (mouille(graphe, u) && get_arc(graphe, u, v)));
...
void tri_topologique(t_gra graphe) //DONE
{
  ...
  do
  {
    ...
    else
    {
      ...
      for (i = 0; i < taille_graphe(graphe); i++)
      {
        aNodeDoesntHaveWeight = aNodeDoesntHaveWeight || !sommet_possede_poids(graphe, i);
      }
    }
...
void multiplie(t_gra graphe)
{
  ...
  //on réinitialise les couleurs a NOIR
  ...
  while (K < taille_graphe(graphe) - 1)
  {
    ...
    for (u = 0; u < taille_graphe(copieGraphe); u++)
    {
      ...
      for (v = 0; v < taille_graphe(copieGraphe); v++)
      {
        //on enlève les cas non pertinents (reflexif et get_arc(u,v)=1)
        if (u != v && !get_arc(graphe, u, v))
        {
          //pour k
          for (k = 0; k < taille_graphe(copieGraphe); k++)
          {
            //Si un arc peut exister, on le créée
  ...
  for (k = 0; k < taille; k++)
  {
    ...
    for (u = 0; u < taille; u++)
    {
      ...
      for (v = 0; v < taille; v++)
      {
        ...
        if (get_arc(copieGraphe, u, k) * get_arc(copieGraphe, k, v) && !get_arc(graphe, u, v))
        {
          set_arc(graphe, u, v, la_couleur());
        }
      }
    ...
    for (u = 0; u < taille_graphe(copieGraphe); u++)
    {
      ...
      for (v = 0; v < taille_graphe(copieGraphe); v++)
      {
        ...
        if (u != v && !get_arc(graphe, u, v))
        {
          ...
          for (k = 0; k < taille_graphe(copieGraphe); k++)
          {
            //on cherche la valeur de l'arc minimum ssi il peut exister
            if (get_arc(copieGraphe, u, k) && get_arc(copieGraphe, k, v))
            {
              mini = min(poids_arc(graphe, u, k) + poids_arc(graphe, k, v), mini);
            }
          }
          //on met l'arc min ssi != +INFINI
          if (mini != PLUS_INF)
          {
...
{ /* +/- 10 lignes. Il faudra retourner une valeur convenable. */
  ...
  for (u = 0; u < taille_graphe(graphe); u++)
  {
    for (v = 0; v < taille_graphe(graphe); v++)
    {
      if (get_arc(graphe, u, v))
      {
        //car la pondération doit être positive
...
{ /* +/- 10 lignes. Il faudra retourner une valeur convenable. */
  ...
  //si on renvoie un sommet, on le trempe
  if (sommet_min != -1)
  {
...
void relax(t_gra graphe, int table_predecesseurs[], int pred, int sommet)
{
  ...
  //si la distance par ce chemin est plus courte
  if (distance < sommet_get_poids(graphe, sommet))
  {
    // si un prédécesseur avait déja été  mis, on met l'ancien en noir
...
void dijkstra(int depart, t_gra graphe, int table_predecesseurs[])
{
  ...
  while (u != -1)
  {
    for (v = 0; v < taille; v++)
    {
      ...
      {
        //il faut que l'arc existe et que la cible soit sèche
...
int cherche_sec_sommet_max(t_gra graphe)
{
  ...
  for (u = 0; u < taille_graphe(graphe); u++)
  {
    ...
    //  Avoir un poids
    //  ne pas etre mouillé
    //  être supérieur au poids max
    //  mais être inférieur a +INFINI
    if (sommet_possede_poids(graphe, u) && (sommet_get_poids(graphe, u) > maxi) && !mouille(graphe, u) && sommet_get_poids(graphe, u) < PLUS_INF)
    {
      //on conserve le max
      sommet_max = u;
                           ...
                           int pred, int sommet, int depart)
{
  ...
  if (pred == depart)
  {
    //Dans le cas ou on part du départ, on met le poids a celui de l'arc
  ...
  else if (sommet_get_poids(graphe, sommet) == PLUS_INF)
  {
    ...
    //Dans le cas où la cible a un poids infini, on met le poids au min.
    distance = min(sommet_get_poids(graphe, pred), poids_arc(graphe, pred, sommet));
    sommet_set_poids(graphe, sommet, distance);
    table_predecesseurs[sommet] = pred;
    definir_couleur(ROUGE);
    set_couleur_arc(graphe, pred, sommet, la_couleur());
...
void adapte_flot(t_gra graphe_flot, int depuis, int vers, int valeur)
{
  ...
  {
    marge = capacite_arc(graphe_flot, depuis, vers) - poids_arc(graphe_flot, depuis, vers);
    if (marge >= valeur)
    {
